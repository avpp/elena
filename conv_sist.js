...
this.prototype.gen_form = function(t) {
  ...
  tsk_el.innerHTML = "Переведите число из одной позиционной системы счисления в другую";
  ...
  o2_el = document.createElement("sub");
  o2_el.innerHTML = t.o2.toString();
  ...
  el.appendChild(ans_el);
  el.appendChild(o2_el);
  return el;
}
...
this.prototype.check = function (task) {
  rslt=parseInt(task.answer, task.o2);
  return (rslt == task.n1);
}
...
this.prototype.get_task_table_view = function(task) {
  ...
  c.innerHTML = task.n1.toString(task.o1) + "<sub>" + task.o1 + "</sub> = ?<sub>" + task.o2 + "</sub>";
  el.appendChild(c);
  c = document.createElement("td");
  c.innerHTML = task.answer;
  el.appendChild(c);
  c = document.createElement("td");
  c.innerHTML = task.n1.toString(task.o1) + "<sub>" + task.o1 + "</sub> = " + task.n1 + "<sub>10</sub>=" + task.n1.toString(task.o2) + "<sub>" + task.o2 + "</sub>";
  el.appendChild(c);
  c = document.createElement("td");
